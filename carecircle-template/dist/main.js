...
async function loadDashboard() {
  ...
  try {
    ...
    if (!patientRelationships || patientRelationships.length === 0) {
      ...
      // Multiple patients - check if we already have one selected
      if (currentPatientId) {
        // Already have a patient selected - just refresh admin status
        const currentRel = patientRelationships.find(r => r.patient_id === currentPatientId);
        if (currentRel) {
          currentPatientName = currentRel.patients?.name || currentPatientName;
          isAdmin = currentRel.is_admin || false;
        }
      } else {
        // No patient selected yet - show selector
...
function renderDashboard() {
  ...
  // Switch Patient dropdown (Admin only)
