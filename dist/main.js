...
let showBMSection = false; // Track if BM section is expanded (collapsed by default)
let showCareTeamSection = false; // Track if care team section is expanded (collapsed by default)
...
function renderDashboard() {
      ...
      <div class="max-w-6xl mx-auto">
            ...
            <div class="bg-slate-800 p-6 rounded-2xl border border-slate-700">
              <div class="flex justify-between items-center mb-4">
                <div class="flex items-center gap-3">
                  <button id="toggle-care-team-btn" class="text-slate-400 hover:text-slate-200 transition-colors">
                    ${showCareTeamSection ? '▼' : '▶'}
                  </button>
              ...
              <div id="care-team-content" class="${showCareTeamSection ? '' : 'hidden'}">
        ...
        <div class="mt-8 bg-slate-800 p-6 rounded-2xl border ${bmBorderClass}">
          <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            <div class="flex items-center gap-3">
              <button id="toggle-bm-section-btn" class="text-slate-400 hover:text-slate-200 transition-colors">
                ${showBMSection ? '▼' : '▶'}
              </button>
              <div>
          ...
          <!-- BM Content (Collapsible) -->
          <div id="bm-section-content" class="${showBMSection ? '' : 'hidden'}">
            <div class="flex gap-3 w-full md:w-auto mb-6">
  ...
  document.getElementById('toggle-care-team-btn')?.addEventListener('click', () => {
    showCareTeamSection = !showCareTeamSection;
    renderDashboard();
  });
  ...
  document.getElementById('toggle-bm-section-btn')?.addEventListener('click', () => {
    showBMSection = !showBMSection;
    renderDashboard();
  });
...
// Note: This function is no longer needed for resetting logs or goals.
// Water and juice logs persist throughout the day and are only filtered by date when displaying.
// Goals persist until the user manually changes them.
// The daily reset at 12:00 AM is handled by the date-based filtering in loadHydrationLogs() and loadJuiceLogs().
async function checkDailyReset() {
  // This function is kept for backward compatibility but does nothing.
  // Logs are filtered by date in loadHydrationLogs() and loadJuiceLogs() using:
  // const today = new Date().toISOString().split('T')[0];
  // .gte('logged_at', today)
  // This ensures only today's logs are displayed, and logs automatically reset at midnight.
  // Goals are persisted in the team_settings table and only change when the user updates them.
}
